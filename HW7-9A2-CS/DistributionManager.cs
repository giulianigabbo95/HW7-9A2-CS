...
namespace MyHomework
{
    ...
    public class Distribution
    {
        ...
        public Distribution(int nbPoints, int nbPaths, double variance = 0.03, double mean = 0)
        {
            mu = mean;
            sigma = variance;
        ...
        public List<RandomPath> GenerateDistribution(int type)
        {
            double lastValue = 0;
            double randomVal = 0;
            double randomStep = 0;
            ...
            for (int i = 0; i < noPaths; i++)
            {
                ...
                for (int x = 1; x <= noPoints; x++)
                {
                    if (type == 0)
                    {
                        randomVal = GetRandomRademacherVariable();
                        randomStep = (sigma * Math.Sqrt(1d / noPoints) * randomVal);
